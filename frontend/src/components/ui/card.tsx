...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        // Mock reference: .card — 16px radius, 20px padding, the one elevation.
        "flex flex-col gap-4 rounded-lg border border-border/80 bg-card py-5 text-card-foreground shadow-card",
        className
      )}
      {...props}
    />

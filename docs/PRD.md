...
| 4 | §81 says real results come from an external API with Resilience4J; the Alcance and every flow describe manual admin loading | **Manual loading only in v1** | Manual loading is the guaranteed path and can never fail. External-provider integration is a designated cut; Resilience4J is exercised by the live Prediction/Score service clients instead. |
